...
class GameBase {
    ...
    private realLogin(desc: string, data?: any) {
        ...
        if (!this.network.connected) {
            ...
            this.network.call_login_parameter(data.type, data.server_name, data.account, data.pwd, WebConfig.inviteCode, WebConfig.systemInfo || "", WebConfig.modelInfo || "", WebConfig.deviceId || "");
            ...
            if (msg.type == Operation_Fields.OPRATE_LOGIN) {//登录操作错误类型
                ...
                switch (msg.reason) {
                    case Operation_Fields.OPRATE_WEB_FAIL_RESULT:             // web返回的错误信息
                        if (msg.data == 33 || msg.data == 164) return;//密码错误和验证码错误不飘字，更换表现
